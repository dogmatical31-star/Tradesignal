...
const runSniperBacktest = (candles) => {
  ...
  for (let i = 65; i < candles.length - 5; i++) {
    ...
    // 음봉이어도 허용 (하락 중 반등 진입이므로 음봉이 정상)
    // 단, 엄청난 장대음봉은 제외 (몸통이 전체 범위의 80% 초과)
    const range = candles[i].high - candles[i].low;
    const body = Math.abs(candles[i].close - candles[i].open);
    if (range > 0 && body / range > 0.85) continue; // 극단적 음봉만 제외
    ...
    // 분할 익절: 1부 +6%, 2부 RSI70 or BB상단
    const entry = candles[i].close;
    const candleLow = candles[i].low;
    // 손절: 진입봉 저가 아래 1% (고정 % 아닌 캔들 구조 기반)
    const stop = Math.min(entry * 0.94, candleLow * 0.99);
    const take1 = entry * 1.06;  // 1부 익절 +6%
  ...
  for (let i = 65; i < candles.length - 5; i++) {
    const r = rsi[i]; if (!r || r > 40) continue; d1++;
    if (!bb[i]?.lower || closes[i] > bb[i].lower * 1.06) continue; d2++;
    if (!sma60[i]) continue;
    const rt = closes[i] / sma60[i]; if (rt < 0.92 || rt > 1.08) continue; d3++;
    if (!sma20v[i] || candles[i].volume < sma20v[i] * 1.2) continue; d4++;
    const rng = candles[i].high - candles[i].low;
    const bdy = Math.abs(candles[i].close - candles[i].open);
    if (rng > 0 && bdy / rng > 0.85) continue; d5++;
    d6++;
  }
  const debugInfo = `캔들:${candles.length}개 | RSI40↓:${d1} | BB:${d2} | 60일선:${d3} | 거래량:${d4} | 양봉:${d5} | 최종:${d6}`;
          ...
          {tab === "sniper" && (
            <div style={{ animation: "fadeUp 0.3s ease" }}>
              ...
              <div style={{ background: G.panel, border: `1px solid ${G.border}`, borderRadius: 12, padding: 16, marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 10, color: G.muted, marginTop: 8, lineHeight: 1.7 }}>
                  📅 일봉 기준 2년치 데이터 · 분할 익절: 1부 +6% / 2부 RSI70 or BB상단 · 손절: 진입봉 저가 -1%
                </div>
              </div>
