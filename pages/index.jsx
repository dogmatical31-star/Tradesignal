...
export default function Home() {
  ...
  const runSN = async () => {
    setSnLoading(true);
    const { candles, source } = await fetchDailyCandles(snTicker);
    setSnResult({ ...runSniperBacktest(candles), source });
    setSnLoading(false);
  };
          ...
          {tab === "sniper" && (
            <div style={{ animation: "fadeUp 0.3s ease" }}>
              ...
              <div style={{ background: G.panel, border: `1px solid ${G.border}`, borderRadius: 12, padding: 16, marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 10, color: G.muted, marginTop: 8, lineHeight: 1.7 }}>
                  📅 일봉 2년치 · RSI바닥 터치 후 반등 확인 진입 · 1부 +6% / 2부 RSI65 or BB상단 · 손절: 진입봉 저가
                </div>
              </div>
