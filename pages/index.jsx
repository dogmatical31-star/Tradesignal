...
const fetchDailyCandles = async (ticker) => {
  try {
    ...
    // 항상 최신 500개로 고정 → 매번 동일한 결과 보장
    const candles = all.slice(-500);
...
const runSniperBacktest = (candles, spyCandles = null) => {
  ...
  for (let i = 65; i < candles.length - 5; i++) {
    ...
    // ③ 눌림목: RSI 42~62 (모멘텀 더 확실할 때)
    if (r < 42 || r > 62) continue;

    // ④ 20일선 근처 지지: 종가가 20일선 ±5% 이내 (완화)
    const sma20Ratio = closes[i] / sma20[i];
    if (sma20Ratio < 0.95 || sma20Ratio > 1.05) continue;
    ...
    // ⑦ 모멘텀: 최근 3일 중 RSI가 최고값 (상승 모멘텀 확인)
    const rsi3 = [rsi[i-2], rsi[i-1], rsi[i]].filter(v => v != null);
    if (rsi3.length < 2 || r !== Math.max(...rsi3)) continue;

    // 분할 익절: 1부 +4%, 2부 +8% 또는 BB상단
    const entry = candles[i].close;
    const stop = entry * 0.96;      // 진입가 -4% 고정 손절 (변동성 흡수)
    const take1 = entry * 1.04;     // 1부 +4%
    const take2 = entry * 1.08;     // 2부 +8%
    let exit1 = null, exit2 = null, reason1 = "", reason2 = "";

    for (let j = i + 1; j < Math.min(i + 25, candles.length); j++) {
      if (candles[j].low <= stop) {
        if (!exit1) { exit1 = stop; reason1 = "손절(-4%)"; }
        if (!exit2) { exit2 = stop; reason2 = "손절(-4%)"; }
        break;
      }
      if (!exit1 && candles[j].high >= take1) { exit1 = take1; reason1 = "익절(+4%)"; }
      if (exit1 && !exit2) {
        if (candles[j].high >= take2) { exit2 = take2; reason2 = "익절(+8%)"; break; }
        ...
        if (sma20[j] && candles[j].close < sma20[j] * 0.97) { exit2 = candles[j].close; reason2 = "손절(20일선)"; break; }
      }
    }
  ...
  for (let i = 65; i < candles.length - 5; i++) {
    ...
    if (r < 42 || r > 62) continue; d3++;
    const rt = closes[i] / sma20[i]; if (rt < 0.95 || rt > 1.05) continue; d4++;
    if (!sma20v[i] || candles[i].volume < sma20v[i] * 0.9) continue; d5++;
    if (candles[i].close <= candles[i].open) continue; d6++;
    const rsi3d = [rsi[i-2], rsi[i-1], r].filter(v => v != null);
    if (rsi3d.length < 2 || r !== Math.max(...rsi3d)) continue; d7++;
  }
...
export default function Home() {
  ...
  const [snCache, setSnCache] = useState({}); // 종목별 캔들 캐시
  ...
  const runSN = async () => {
    setSnLoading(true);
    // 캐시 확인 - 같은 종목은 동일 데이터 재사용
    const cacheKey = snTicker;
    const spyCacheKey = "SPY";
    let candles, source, spyCandles;

    if (snCache[cacheKey]) {
      ({ candles, source } = snCache[cacheKey]);
    } else {
      ({ candles, source } = await fetchDailyCandles(snTicker));
      setSnCache(prev => ({ ...prev, [cacheKey]: { candles, source } }));
    }

    if (snCache[spyCacheKey]) {
      spyCandles = snCache[spyCacheKey].candles;
    } else {
      const spyData = await fetchDailyCandles("SPY");
      spyCandles = spyData.candles;
      setSnCache(prev => ({ ...prev, [spyCacheKey]: spyData }));
    }
            ...
            <div style={{ animation: "fadeUp 0.3s ease" }}>
              ...
              <div style={{ background: "#0a1929", border: "1px solid rgba(79,195,247,0.3)", borderRadius: 12, padding: 14, marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 10, color: G.muted, lineHeight: 1.8 }}>
                  ...
                  <span style={{ color: G.yellow }}>📉 눌림목</span>: RSI 42~62 + 20일선 ±5% + 양봉 + 3일 RSI 최고<br />
              ...
              <div style={{ background: G.panel, border: `1px solid ${G.border}`, borderRadius: 12, padding: 16, marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
                  {tickers.map(t => (
                    <button key={t} onClick={() => { setSnTicker(t); setSnResult(null); }} style={{ padding: "6px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600, background: snTicker === t ? "rgba(79,195,247,0.15)" : G.dim, border: `1px solid ${snTicker === t ? "rgba(79,195,247,0.4)" : G.border}`, color: snTicker === t ? G.blue : G.muted }}>{t}</button>
                  ))}
                </div>
                ...
                <div style={{ fontSize: 10, color: G.muted, marginTop: 8, lineHeight: 1.7 }}>
                  📅 일봉 2년치 · 상승장 눌림목 전략 · SPY+종목 60일선 위 · RSI 42~62 · 20일선 ±5% · 1부 +4% / 2부 +8% · 손절: -4%
                </div>
              </div>
