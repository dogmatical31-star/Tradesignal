...
/* ── 스나이퍼 백테스트 v2: RSI 바닥 터치 후 반등 확인 진입 ── */
const runSniperBacktest = (candles, spyCandles = null) => {
  ...
  // SPY 시장 추세 - 날짜 인덱스 기반 정렬 (날짜 문자열 매칭 대신 timestamp 사용)
  const spyUpArr = [];
  if (spyCandles && spyCandles.length > 60) {
    ...
    spyCandles.forEach((c, i) => {
      spyUpArr.push({
        ts: c.timestamp,
        up: spySma20[i] != null && spySma60[i] != null
          ? spyCloses[i] > spySma20[i] && spySma20[i] > spySma60[i] * 0.99
          : null,
      });
    });
  }
  const hasSpyData = spyUpArr.length > 0;

  // 종목 날짜와 SPY 날짜를 timestamp로 매칭하는 함수
  const isMarketUp = (ts) => {
    if (!hasSpyData) return true;
    // 가장 가까운 SPY 날짜 찾기 (±2일 이내)
    const match = spyUpArr.find(s => Math.abs(s.ts - ts) < 2 * 86400000);
    if (!match || match.up === null) return true; // 데이터 없으면 허용
    return match.up;
  };

  for (let i = 66; i < candles.length - 5; i++) {
    const r = rsi[i];
    const rPrev = rsi[i - 1];
    if (!r || !rPrev) continue;

    // 🌍 시장 필터: SPY 상승장일 때만 진입
    if (!isMarketUp(candles[i].timestamp)) continue;

    // ① RSI 바닥 확인: 최근 6일 중 RSI 32 이하 터치했어야 함
    const recentRsi = rsi.slice(Math.max(0, i - 6), i);
    if (!recentRsi.some(v => v != null && v <= 32)) continue;

    // ② 반등 확인: 현재 RSI 35~55 사이이고 전날보다 상승
    if (r < 35 || r > 55 || r <= rPrev) continue;
    ...
    // ④ 볼린저 하단 ~ 중심선 사이
    if (!bb[i].lower || !bb[i].mid) continue;
    if (closes[i] > bb[i].mid * 1.02) continue;

    // ⑤ 거래량 평균 이상
    if (!sma20v[i] || candles[i].volume < sma20v[i] * 0.8) continue;
    ...
    const stop = candles[i].low * 0.985; // 진입봉 저가 아래 1.5%
    const take1 = entry * 1.06;          // 1부 +6%
    ...
    for (let j = i + 1; j < Math.min(i + 25, candles.length); j++) {
      ...
      if (!exit1 && candles[j].high >= take1) { exit1 = take1; reason1 = "익절(+6%)"; }
      if (exit1 && !exit2) {
        if (rsi[j] != null && rsi[j] >= 65) { exit2 = candles[j].close; reason2 = "익절(RSI65)"; break; }
        if (bb[j].upper && candles[j].high >= bb[j].upper) { exit2 = bb[j].upper; reason2 = "익절(BB상단)"; break; }
        if (sma60[j] && candles[j].close < sma60[j] * 0.95) { exit2 = candles[j].close; reason2 = "손절(60일선)"; break; }
      }
    }
  ...
  for (let i = 66; i < candles.length - 5; i++) {
    ...
    if (!r || !rPrev) continue;
    const recentRsi = rsi.slice(Math.max(0, i - 6), i);
    if (!recentRsi.some(v => v != null && v <= 32)) continue; d1++;
    if (r < 35 || r > 55 || r <= rPrev) continue; d2++;
    if (!sma60[i]) continue;
    const rt = closes[i] / sma60[i]; if (rt < 0.88 || rt > 1.12) continue; d3++;
    if (!bb[i]?.lower || !bb[i]?.mid || closes[i] > bb[i].mid * 1.02) continue; d4++;
    if (!sma20v[i] || candles[i].volume < sma20v[i] * 0.8) continue; d5++;
    if (candles[i].close <= candles[i].open) continue; d6++;
  }
  const debugInfo = `캔들:${candles.length}개 | RSI바닥:${d1} | 반등:${d2} | 60일선:${d3} | BB위치:${d4} | 거래량:${d5} | 양봉:${d6}`;
          ...
          {tab === "sniper" && (
            <div style={{ animation: "fadeUp 0.3s ease" }}>
              ...
              <div style={{ background: G.panel, border: `1px solid ${G.border}`, borderRadius: 12, padding: 16, marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 10, color: G.muted, marginTop: 8, lineHeight: 1.7 }}>
                  📅 일봉 2년치 · SPY 상승장 필터 · RSI바닥 터치 후 반등 진입 · 1부 +6% / 2부 RSI65 or BB상단 · 손절: 진입봉 저가
                </div>
              </div>
