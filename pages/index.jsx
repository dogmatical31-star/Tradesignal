...
const runSniperBacktest = (candles, spyCandles = null) => {
  ...
  if (spyCandles && spyCandles.length > 60) {
    ...
    spyCandles.forEach((c, i) => {
      if (spySma20[i] != null && spySma60[i] != null) {
        spyUpMap[c.time] = spyCloses[i] > spySma20[i] && spySma20[i] > spySma60[i] * 0.99;
      }
    });
  }
  const hasSpyData = Object.keys(spyUpMap).length > 0;
  ...
  for (let i = 66; i < candles.length - 5; i++) {
    ...
    if (hasSpyData && spyUpMap[candles[i].time] === false) continue;
